...
serve(async (req) => {
  ...
  try {
    ...
    if (!content || content.trim().length === 0) {
      return new Response(
        JSON.stringify({ error: "Content is required to generate an exam" }),
...
${enabledTypes.includes("fillInBlank") ? '- fillInBlank: Student types the answer. Use "correct_answers" array for accepted variations (e.g., ["photosynthesis", "photo-synthesis"]).' : ""}
...
${enabledTypes.includes("graph") ? '- graph: A question based on a mathematical graph. Include "graphData" with functions, points, shapes, and config.' : ""}
...
Make the exam progressively harder. Start with easier questions and build up.`;
    ...
    console.log(`Successfully generated exam with ${exam.questions?.length || 0} questions`);
